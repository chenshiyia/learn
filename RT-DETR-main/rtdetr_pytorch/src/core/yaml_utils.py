...
def load_config(file_path, cfg=dict()):
    ...
    with open(file_path) as f:
        file_cfg = yaml.load(f, Loader=yaml.Loader) # load方法加载读取路径所在文件

        if file_cfg is None:
            return {}
        # 全局变量，用于在yaml转化的字典中寻找其对应的值
    if INCLUDE_KEY in file_cfg:
        # 如果yaml文件中有include，将对应的yaml文件加载到列表中，file_cfg[INCLUDE_KEY]是'__include__'对应的值
